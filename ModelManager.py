...
class ModelManager:
    ...
    @staticmethod
    def darwinism():
        next_generation_models = rank_models(ModelManager.models, ModelManager.symbol_sequences)
        ...
        next_generation_models = next_generation_models[:-int(reproduction_number / 2)] # remove weakest models which will the be replaces by offsprings

        for i in range(0, reproduction_number, 2):
            # create offsprings and add them to Models

            offspring = create_offspring(next_generation_models[i], next_generation_models[i+1])
